...
import { X, Plus, Trash2, RotateCcw, Volume2 } from 'lucide-react-native';
...
export default function SettingsScreen() {
  ...
  const { cycles, rewards, updateCycle, addCycle, removeCycle, updateRewards, resetSettings, theme, themeName, setThemeName, dailyGoalMinutes, setDailyGoalMinutes, alarmSound, setAlarmSound } = useSettings();
  ...
  const playSystemPreview = async () => {
    try {
      ...
      await Notifications.scheduleNotificationAsync({
        ...
        android: { channelId: 'timer-default' },
      });
    } catch (error) {
      console.log('Error playing system preview', error);
    }
  };
        ...
        <View style={styles.section}>
          ...
          <View style={styles.soundOptions}>
            {[
              ...
              { id: 'system', label: 'Som do sistema', description: 'Padrão do dispositivo' },
              ...
              return (
                <PressableScale
                  ...
                  onPress={() => setAlarmSound(option.id as 'alarm' | 'system' | 'silent')}
                >
                  <View>
                  ...
                  <View style={styles.soundRight}>
                    ...
                    {option.id === 'system' && (
                      <Pressable onPress={playSystemPreview} style={styles.soundPreview}>
                        <Volume2 color={theme.colors.accent} size={16} />
                      </Pressable>
                    )}
